			...
			if (!err) {
				if (!serviceData) {
					request('http://www.transitlink.com.sg/eservice/eguide/service_route.php?service=' + service, (err, res) => {
						...
						var stopList = Array.from(dom.window.eval(`document.querySelectorAll('#calculateFareDir1 > section:nth-child(5) > table:nth-child(1) > tbody:nth-child(1) tr')`));
						stopList = stopList.slice(3, -1).filter(element => {
