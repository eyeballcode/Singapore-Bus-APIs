...
class TransitLink {
	...
	operationSpecs.operationTimings[day][direction].firstBus = this.getServiceInfoTable
(dom, 2 + direction, column)
	...
	this.serviceInfo.find({
		...
		if ((new Date() - serviceData.asOf) > 86400000 && serviceData.type === 'auto') {
		    this.serviceInfo.remove({service: serviceData}, () => {
	...
	this.meta.find({
		...
		if ((new Date() - services.asOf) > 86400000) {
		    this.meta.remove({type: 'service-list'}, () => {
